...
def create_map_figure(selected_region_name=None):
    ...
    # Add all branches, ensuring legends remain visible
    ...
    for name in unique_names:
        ...
        if selected_region_name:
            # Create a boolean mask to match branches in the selected region
            is_in_region = branches_with_regions['ERNAME'] == selected_region_name
            is_in_region_and_name = is_in_region & (cu_branches1['Name'] == name)
            branch_data['marker_opacity'] = 0.0
            branch_data.loc[is_in_region_and_name, 'marker_opacity'] = 1.0
        else:
            branch_data['marker_opacity'] = 1.0
        
        fig.add_trace(go.Scattermapbox(
            lat=branch_data["Lat"],
            lon=branch_data["Long"],
            mode='markers',
            marker=go.scattermapbox.Marker(size=10, color=color_map[name], opacity=branch_data['marker_opacity']),
            name=name,
